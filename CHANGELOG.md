...
New Features
1. Added Product Hunt badge to README and landing page.
...
docker pull ghcr.io/yogasw/beo-echo:3.0.4
...
New Features
1.  Removed the pricing section and all related navigation links from the landing page.
2.  Implemented replay folder management and expanded replay update capabilities with additional response details.
3.  Introduced nested replay responses with dedicated list display, drag-and-drop, and editing capabilities, alongside a new save response action.
4.  Implemented multi-level drag and drop reordering and folder nesting using depth calculation and visual indicators.
...
```bash
# Pull specific version
docker pull ghcr.io/yogasw/beo-echo:3.0.2
...
### New Features
1. Display build time and link to changelog in the project list version display.
2. Implement save (create and update) functionality for replay requests, supporting flexible metadata and configuration types.
3. Add right-click context menu to replay tabs for duplication, closing, and managing multiple tabs.
4. Implement replay folder creation and integrate folders into the replay listing and management UI.
5. Implement recursive deletion for replay folders, including backend logic, API endpoint, and frontend integration.
6. Add drag-and-drop functionality to move replays and folders, supported by new folder update API endpoints.
7. Implement hierarchical replay list with folder collapse, depth indentation, and visual guides.
8. Implement inline folder creation directly within the replay list UI.
9. Add documentation fields to replay and replay folder models and their associated creation and update requests.
10. Add folder view for replay management with editable title and documentation tab.
11. Integrate folder overview directly into replay editor tabs and update tab types to support folder items.
12. Persist replay folder documentation to backend via API and update UI to use new API method.
13. Implement inline title editing for replay items and folders.

### Improvements
1. Introduce a protocol-agnostic executor interface and dedicated models for replay execution.
2. Improve replay editor tab management by handling empty states and intelligently replacing unedited tabs.
3. Prevent spurious `tabContentChange` dispatches by tracking the last dispatched content and tab ID.
4. Improve replay tab management by replacing empty default tabs, generating more unique IDs, and activating the editor view upon addition.
5. Disable and style save button when no unsaved changes on the active tab, in addition to the loading state.
6. Migrate components to Svelte 5 runes and enhance replay list item display.
7. Remove unused imports and improve z-index and styling for replay list context menu.
8. Reimplement replay list item menu as a global context menu and introduce a new `folder-size` for HTTP method badges.
9. Update replay UI components to use theme-aware utility classes for improved styling consistency.

### Bug Fixes
1. Prepend "http://" to target URLs if a scheme is missing before parsing and logging.
...
```bash
# Pull specific version
docker pull ghcr.io/yogasw/beo-echo:3.0.0
...
## v2.7.4
_Released on 2026-03-08_

**Improvements**
1.  Removed `wails` desktop components.
2.  Updated Go version to 1.26.1 and updated the Go base image to 1.26.1-alpine in both Dockerfiles.
3.  Updated various dependencies, including downgrading frontend `axios`, upgrading backend `protobuf`, and updating `gin`, `gin-contrib/cors`, `golang.org/x/crypto`, SvelteKit, and Svelte.
4.  Implemented a non-root `appuser` with UID 1000 in the Dockerfile for enhanced security.
5.  Improved CI/CD workflows by pinning GitHub Actions to specific versions, refactoring the Docker publish workflow to pass variables via environment and improve multi-line output generation, reordering Docker build and push steps, using GitHub Actions variables for `GEMINI_MODEL` instead of secrets, and ensuring the `trigger-deployment` job runs only after a successful public release.
6.  Enhanced prerendering by gracefully handling unseen route errors and reducing the list of prerendered entries for landing mode.

**Bug Fixes**
1.  Added path validation to prevent directory traversal in Caddy config generation.
2.  Addressed security vulnerabilities by pinning 3rd party GitHub Actions.
3.  Improved robustness and error handling for Gemini summary generation in the Docker publish workflow, including better API secret validation and `curl` error handling.
...
```bash
# Pull specific version
docker pull ghcr.io/yogasw/beo-echo:2.7.4
...
### Improvements
1.  Updated security notice in  to clarify sandbox limitations.
### Other
1.  Updated  for v2.6.0.
2.  Bumped version to 2.6.1.
### Docker Images
Use one of these commands to pull the Docker image:
...
## v2.6.0
_Released on 2025-11-10_

**New Features**

1.  Added a new action feature.
2.  Implemented a JavaScript action.
3.  Introduced an action for documentation.

**Improvements**

1.  Refactored code by replacing the default logger with a custom logger and incorporating recovery middleware.
2.  Enhanced logging by including request IDs.
3.  Restructured the action folder organization.
4.  Improved the listing action functionality.
5.  Improved the header creation action.
6.  Improved the header and list actions.
7.  Improved overall code structure.
8.  Revamped the action form.
9.  Enhanced the replace text validation process.
10. Updated the priority action.

**Bug Fixes**

1.  Fixed issues with pipelines.
### Docker Images
Use one of these commands to pull the Docker image:
...
## v2.5.3
_Released on 2025-11-03_

**New Features**

1.  Adds a click handler to the logo that resets the project selection and clears logs.
2.  Implements a Tooltip component in the Header for improved user guidance.

**Improvements**

1.  Enhances the layout and stats overview of the UnSelectedProject component.
